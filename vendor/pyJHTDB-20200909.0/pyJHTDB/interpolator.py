...
class spline_interpolator:
    ...
    def cinterpolate(
        ...
        if not self.clib_loaded:
            self.generate_clib()
        diff = np.array(diff).astype(int32)
        field_offset = np.array(field_offset).astype(int32)
        field_size   = np.array(f.shape[:-1]).astype(int32)
        ...
        assert(f.flags['C_CONTIGUOUS'] and
               f.dtype == float32 and
               len(f.shape) == 4)
        y = np.ascontiguousarray(x.reshape(-1, 3), float32)
        node_array = np.zeros(y.shape, int32)
        ...
        if debug:
            print(node_array)
            print(field_offset)
        s = np.ascontiguousarray(np.zeros((y.shape[0], f.shape[-1]), float32))
