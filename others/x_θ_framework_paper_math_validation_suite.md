...
\[
\oint \mathbf{A}\cdot d\mathbf{x} \approx \sum_{i=0}^{N-1} \mathbf{A}(\mathbf{x}_i)\cdot(\mathbf{x}_{i+1}-\mathbf{x}_i).
\]

Use midpoint evaluation \(\mathbf{A}((\mathbf{x}_i+\mathbf{x}_{i+1})/2)\) for better accuracy.

---

# Appendix B — Derivation: CHSH for \(E(\Delta)=\cos(2\Delta)\)

With \(a=0\), \(a'=\pi/4\), \(b=\pi/8\), \(b'=3\pi/8\):

\[
E(a,b)=\cos\left(2\left(\frac{\pi}{8}\right)\right)=\cos\left(\frac{\pi}{4}\right)=\frac{\sqrt{2}}{2}
\]

\[
E(a,b')=\cos\left(2\left(\frac{3\pi}{8}\right)\right)=\cos\left(\frac{3\pi}{4}\right)=-\frac{\sqrt{2}}{2}
\]

\[
E(a',b)=\cos\left(2\left(\frac{\pi}{8}-\frac{\pi}{4}\right)\right)=\cos\left(-\frac{\pi}{4}\right)=\frac{\sqrt{2}}{2}
\]

\[
E(a',b')=\cos\left(2\left(\frac{3\pi}{8}-\frac{\pi}{4}\right)\right)=\cos\left(\frac{\pi}{4}\right)=\frac{\sqrt{2}}{2}
\]

Then

\[
S = \left|\frac{\sqrt{2}}{2} + \left(-\frac{\sqrt{2}}{2}\right) + \frac{\sqrt{2}}{2} - \frac{\sqrt{2}}{2}\right| = 0.
\]

This indicates the sign convention must be handled carefully: CHSH uses different combinations depending on the experimental mapping from \(\Delta\) to each \(E\). In practice one chooses the standard polarization correlation \(E=\cos 2(a-b)\) **and** the CHSH combination

\[
S = |E(a,b)-E(a,b')+E(a',b)+E(a',b')|
\]

which yields \(2\sqrt{2}\) for the standard angles. The implementation section should explicitly define the CHSH sign convention used.
