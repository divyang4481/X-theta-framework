\section{The X--$\theta$ Mathematical Framework (Central Formalism)}\label{sec:framework}
...
\begin{idea}
Think of ordinary space as a city map, and add a tiny circular \emph{dial} $\theta$ at every point. Turning the dial moves you along the fiber circle without moving on the map. The connection $A$ tells you how “phase” changes as you move—on the map and around the dial. The curvature $G=dA$ measures how those changes \emph{fail to cancel} around a loop (that failure is the loop phase / transport phase). A picture of this "map\,$\times$\,dial" view is shown in Fig.~\ref{fig:map-dial}.
\end{idea}
...
\begin{itemize}
  ...
  \item \textbf{Curvature ($G=dA$) and loop phase.} Curvature measures the failure of phase changes to cancel on a loop; the loop phase is the loop-induced transport phase. Analogy: hike a loop around a hill and your compass heading can twist.
...
With Newtonian time $t$ and scalar potential $A_0$,
\begin{align}
...
Rotor spectrum and loop-phase shift:
\begin{equation}
...
\paragraph{Massless limit ($m\to 0$).} With finite $\kappa_0$,
\begin{equation}
...
\subsection{Gauge invariance on $Q$ and large loops}\label{sec:gauge-invariance}
...
\begin{figure}[htbp]
  ...
  \caption{Gauge change shifts the reference, not the hill. A closed loop probes the loop phase; only $\phi_\theta\ (\bmod\ 2\pi)$ is observable.}
  \label{fig:ug-loopphase}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Loop-phase schematic: Stokes rectangles visualize how loop integrals relate to enclosed curvature; only the loop integral modulo the large-gauge period is observable.}
  \label{fig:stokes}
\end{figure}
...
I summarize the $I\to\infty$, $q_X\to 0$, and $I=m\kappa^2$ limits and their outcomes for observables (sidebands, loop phase) and consistency.
