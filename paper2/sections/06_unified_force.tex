\section{Consistent Relativistic Completion (Non-Minimal Extension)}\label{sec:unified-force}

\begin{quote}
\emph{Disclaimer: This section demonstrates that the X--$\theta$ framework admits a relativistically consistent completion. It does not assert uniqueness or phenomenological necessity, but serves to show that the framework is compatible with standard 4D field theory constraints.}
\end{quote}
...
\subsection{From $Q$ to 4D: fields and covariant derivatives}
...
\subsection{Distance-law modifications with a shared range $\lambda_\theta$}
...
See also the rotor ladder shift in Fig.~\ref{fig:ug-rotor} for an intuitive picture of the loop-phase shift.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{UG_S3_rotor_ladder.png}
  \caption{Rotor ladder with loop-phase shift: the minimum moves by $\phi_\theta/2\pi$.}
  \label{fig:ug-rotor}
\end{figure}
