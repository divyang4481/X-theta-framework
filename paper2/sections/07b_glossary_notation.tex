...
\begin{description}
  ...
  \item[$A_\mu,\ A_0,\ \mathbf B$] Spacetime potential $A_\mu=(A_0,\mathbf A)$ with $\mathbf B=\nabla\times \mathbf A$.
  \item[$G=dA$] Curvature (field strength) 2-form with components $G_{ab}=\partial_aA_b-\partial_bA_a$; controls the loop phase via Stokes' theorem.
  ...
  \item[Loop phase] Loop-induced transport phase; for the fiber it is $\phi_\theta$ above. Only $\phi_\theta\ (\bmod\,2\pi)$ is observable.
    \begin{itemize}
...
\end{description}
  ...
  % Compact add-ons requested by user
  \paragraph{Loop phase.}
