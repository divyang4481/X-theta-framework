#!/usr/bin/env python3
"""
weihs_universal_pipeline_v13.py

THE UNIFIED DISCOVERY PIPELINE (Memory-Safe Ensemble)
================================================================================
CHANGES FROM v12:
  1. MEMORY FIX: Reduced synchronization sample size from 50,000 to 10,000.
     v12 attempted to allocate ~20GB RAM per file (50k^2 matrix), causing 
     system freeze. v13 uses ~800MB RAM (10k^2), which is safe and fast.
  2. PROGRESS BAR: Prints every file processed so you know it's not stuck.
  3. ENSEMBLE LOGIC: Keeps the strategy of mining ALL data with the known 
     v19 config (Set=0, Det=1) to check global S.

USAGE:
   python weihs_universal_pipeline_v13.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
CACHE_FILE = "weihs_mined_data_v13.npz"
...
def find_shift_and_sync(tA, tB):
    # LIMIT to 10,000 events to prevent RAM explosion (10k^2 doubles = 800MB)
...
def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Ensemble Miner v13 (Memory Safe)...")
    ...
    with zipfile.ZipFile(alice_zip, "r") as za, zipfile.ZipFile(bob_zip, "r") as zb:
        ...
        for i, stem in enumerate(stems):
            # Print every file so user knows it's working
            if i % 10 == 0: print(f"  Scanning [{i}/{len(stems)}]: {stem[:20]}...")
            
            tA, cA = get_raw_buffers(za, stem)
            tB, cB = get_raw_buffers(zb, stem)
            if tA is None or len(tA) < 500: continue
            
            # Sync (Now safe with 10k limit)
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v13")
    print("==========================================")
