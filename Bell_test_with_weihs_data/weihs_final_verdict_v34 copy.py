...
THE RESCUE MISSION: RESTORING BASELINE S
================================================================================
PRIORITY:
  Get Baseline S > 2.3. (Currently ~1.95 in v32).
  Without S > 2, there is no quantum violation to analyze.

UPGRADES:
  1. SYMMETRIC SHIFT ESTIMATOR: Checks both neighbors (j, j-1) to lock sync.
  2. PER-STEM DIAGNOSTICS: Prints S for every file. identifies "rotten" stems.
  3. STRICT 4-CORNER TOPOLOGY: Kept from v32 (correct theory).
  4. DRIFT-PRESERVING NULL: Kept from v32 (correct stats).

USAGE:
  python weihs_final_verdict_v34.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --path-contains "timetags/longdist" --window 3.5 --mode greedy
"""
...
def estimate_shift_ns(tA, tB):
    # UPGRADE: Symmetric Neighbor Check
    ...
    hist, edges = np.histogram(dt[mask], bins=2000) # High res
...
def main():
    ...
    ap.add_argument("--mode", type=str, default="greedy", choices=["strict", "greedy"])
    ap.add_argument("--force-remine", action="store_true")
    args = ap.parse_args()

    print(f"=== VERDICT v34 (The Rescue): Window={args.window}ns | Mode={args.mode} ===")
    ...
    with zipfile.ZipFile(args.alice_zip,"r") as za, zipfile.ZipFile(args.bob_zip,"r") as zb:
        ...
        if not cfg or cfg["S"] < 2.0:
            print("CRITICAL: Calibration failed to find S > 2.0. Dataset quality likely poor.")
            # We proceed anyway to show the Per-Stem S debug info
            if not cfg: cfg = {"sb":0, "db":1, "invS_A":True, "invS_B":False, "flip":-1}

        # 2. MINING & DIAGNOSTICS
    ...
    if not ph_l:
        print("CRITICAL: No valid data after filtering.")
        return
    ...
    # PERMUTATION TEST
    print(f"\n[TEST 2] Drift-Preserving Permutation ({PERM_ROUNDS} rounds)...")
    better_count = 0
