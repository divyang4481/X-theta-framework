#!/usr/bin/env python3
"""
weihs_final_verdict_v31.py

THE FINAL VERDICT (Bug Fix: Added BLOCK_SIZE)
================================================================================
FIXES:
  - Added missing 'BLOCK_SIZE' constant.
  - Added missing 'build_events' function.
  - Restored v26 'norm_member' logic to preserve file paths.

USAGE:
  python weihs_final_verdict_v31.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --path-contains "timetags/longdist" --window 3.5 --mode strict
"""
...
KAPPA = 0.1
BIN_COUNT = 8
BLOCK_SIZE = 5000  # <--- Added missing constant
...
def _norm_member(name: str) -> str:
    # RESTORED V26 LOGIC (Preserves Path)
...
def main():
    ...
    print(f"=== VERDICT v31: Window={args.window}ns | Mode={args.mode} ===")
    ...
    with zipfile.ZipFile(args.alice_zip,"r") as za, zipfile.ZipFile(args.bob_zip,"r") as zb:
        ...
        print(f"Found {len(stems)} stems matching '{args.path_contains}'.")
    ...
    print(f"[TEST 2] Block-Shuffle: DeltaChi2 = {block_score:.2f}")
    
    print("\n=== INTERPRETATION ===")
    if real_score > 10.0 and block_score < 0.5*real_score:
        print("SUCCESS: Strong Signal confirmed (Geometric).")
    elif real_score > 10.0 and block_score > 0.5*real_score:
        print("FAILURE: Signal exists but is likely DRIFT.")
    else:
        print("FAILURE: No significant signal found.")
