#!/usr/bin/env python3
"""
weihs_final_verdict_v33.py

THE RESURRECTION: AUTO-CALIBRATION
================================================================================
DIAGNOSIS OF V32 FAILURE:
  - Baseline S was < 2.0 (1.95), proving the hardcoded bit-settings were WRONG.
  - Without a valid Bell violation (S > 2), no geometric phase can be observed.

FIXES IN V33:
  - RESTORED AUTO-CALIBRATION: Scans all 128 bit-configurations to find the
    one that yields the true Bell violation (expect S > 2.5).
  - Uses that optimal config to run the Geometric Phase test.

USAGE:
  python weihs_final_verdict_v33.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --path-contains "timetags/longdist" --window 3.5 --mode greedy
"""
...
def match_pairs(tA, tB, shift, window, mode="strict"):
    ...
    if mode == "greedy":
        # Greedy: Allow reuse, pick closest per A
...
def calibrate_config(za, zb, stems, window, mode):
    ...
    # Check first 3 stems only for speed
    test_stems = stems[:3]
    ...
    for s in test_stems:
        ...
        if len(ai) < 100: continue
        data_cache.append((cA, cB, ai, bi))
        
    if not data_cache: return None

    # Scan
    for sb in [0]: # Usually 0 or 1
        for db in [0, 1]:
            if sb==db: continue
...
def main():
    ...
    print(f"=== VERDICT v33 (Auto-Calib): Window={args.window}ns | Mode={args.mode} ===")
    ...
    with zipfile.ZipFile(args.alice_zip,"r") as za, zipfile.ZipFile(args.bob_zip,"r") as zb:
        ...
        if not cfg or cfg["S"] < 2.0:
            print("CRITICAL: Calibration failed to find S > 2.0.")
            if not cfg: return
    ...
    if p_val < 0.05 and S_global > 2.0:
        ...
        print("FAILURE: No valid discovery.")
