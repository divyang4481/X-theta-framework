#!/usr/bin/env python3
"""
weihs_paper_plot_v19.py

THE RIGOROUS PLOTTER (Low-Count Adaptation)
================================================================================
VERSION HISTORY:
  v19: ADAPTED THRESHOLDS. The diagnostic run revealed a significant imbalance 
       in setting choices (Set00 counts are ~10x lower than Set11). 
       Adjusted MIN_COUNTS_PER_SETTING from 20 -> 5 to allow data to flow 
       without suppressing the plot, while relying on Bootstrap to correctly 
       show the increased uncertainty.
  v18: Added Bin Population Diagnostics.
  v17: Implemented Strict Binning and Holonomy tracking.

SCIENTIFIC CONTEXT:
  This script analyzes Bell Test data using the "X-Theta" framework.
  It tracks the "history" of settings to calculate a geometric phase (Theta).
  
  THE IMBALANCE ISSUE:
  In the Weihs dataset processed here, the '00' setting coincidence rate is 
  anomalously low compared to '11'. This script now accepts this asymmetry
  but will generate larger error bars for terms dependent on E(0,0).

USAGE:
   python weihs_paper_plot_v19.py --alice-zip "A.zip" --bob-zip "B.zip" --reverse-bits
================================================================================
"""
...
MIN_COUNTS_PER_SETTING = 5  # LOWERED FROM 20 TO 5 to accommodate Set00 scarcity
...
def main():
    ...
    print("==========================================")
    print("    WEIH Paper Plotter v19 (Active)       ")
    print("==========================================")
    ...
    print("\n[DIAGNOSTICS] Bin Population Report (Threshold=5):")
    ...
    if not has_valid_data:
        print("\nCRITICAL WARNING: No bins met the requirements.")
    else:
        print("\nSUCCESS: Sufficient data found. Generating Plot...")
    ...
    # Save
    out_name = "weihs_paper_plot_v19.png"
