#!/usr/bin/env python3
r"""
weihs_universal_pipeline_v20.py  (CV Align + cache backward-compat)
...
Usage:
  python weihs_universal_pipeline_v20.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --path-contains "timetags/longdist" --inspect-c

  python weihs_universal_pipeline_v20.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --path-contains "timetags/longdist" --kappa 0.8 --bin-count 12 --perm-rounds 2000
"""
...
def main():
    ...
    ap.add_argument("--cache", default="weihs_mined_v20.npz")
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v20 (CV Align)")
    print("==========================================")
    ...
    with zipfile.ZipFile(args.alice_zip, "r") as za, zipfile.ZipFile(args.bob_zip, "r") as zb:
        ...
        out_png_unaligned = f"weihs_v20_unaligned_k{args.kappa}_b{args.bin_count}.png"
        save_fit_plot(out_png_unaligned, obsA["bin_centers"], obsA["s_vals"], obsA["s_errs"], fitA["beta"],
                      f"Unaligned (bins={args.bin_count}) Δχ²={fitA['delta_chi2']:.2f}")

        permA = permutation_test_stratified(phases_all, ks_all, prods_all, args.bin_count, args.perm_rounds,
        ...
        # FIX: Include kappa and bin_count in filename
        out_png_cvaligned = f"weihs_v20_cvaligned_k{args.kappa}_b{args.bin_count}.png"
        save_fit_plot(out_png_cvaligned, obsB["bin_centers"], obsB["s_vals"], obsB["s_errs"], fitB["beta"],
                      f"CV-aligned held-out (bins={args.bin_count}) Δχ²={fitB['delta_chi2']:.2f}")

        permB = permutation_test_stratified(ph_cv, ks_cv, pr_cv, args.bin_count, args.perm_rounds,
