#!/usr/bin/env python3
"""
weihs_paper_plot_v16.py

Fixes:
1. Path Resolution Bug (now correctly matches 'longdist0' to 'Timetags/longdist/longdist0').
2. Uses Kappa=0.8 (The Validation Winner) instead of 1.25.

python weihs_paper_plot_v16.py   --alice-zip "..\Bell_data\7185335\Alice.zip"   --bob-zip "..\Bell_data\7185335\Bob.zip"   --reverse-bits
"""
...
# === CONFIG ===
# We use 0.8 because it passed the Split-Half Validation
RESONANCE_KAPPA = 0.8 
BOOTSTRAP_ROUNDS = 50
...
def main():
    ...
    with zipfile.ZipFile(args.alice_zip, "r") as za, zipfile.ZipFile(args.bob_zip, "r") as zb:
        # Build a robust map of short_name -> full_path
        # E.g. "longdist0" -> "Timetags/longdist/longdist0"
        full_paths = []
        for n in za.namelist():
             if n.lower().endswith("_v.dat"):
                 ...
                 # Store "Timetags/longdist/longdist0"
                 full_paths.append(cl[:-6])
        
        for item in stems:
            short, d_str = item.split(":")
            # Find the path that ENDS with our short name
    ...
    # Use RAW STRING for latex to avoid warnings
    ...
    plt.savefig("weihs_paper_plot_v16.png")
    print("\nSaved: weihs_paper_plot_v16.png")
