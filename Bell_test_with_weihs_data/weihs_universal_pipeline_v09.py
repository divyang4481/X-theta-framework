#!/usr/bin/env python3
"""
weihs_universal_pipeline_v09.py

THE UNIFIED DISCOVERY PIPELINE (Window-Scanning Calibration)
================================================================================
CHANGES FROM v08:
  1. WINDOW SCANNING: The calibration tournament now tests multiple coincidence 
     windows (2.5ns, 5ns, 12ns) for each file. This handles cases where 'longdist' 
     files have higher timing jitter than expected.
  2. INCREASED CALIBRATION DEPTH: Scans 50,000 events per file (instead of 25k) 
     to reduce statistical noise during the S-calculation.
  3. STRICTER FILTERS: Rejects any result where S < 1.5 to prevent locking onto 
     noise artifacts.

USAGE:
   python weihs_universal_pipeline_v09.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
CACHE_FILE = "weihs_mined_data_v9.npz"
...
def check_file_for_S(za, zb, target):
    """
    Helper: Tries to calculate max S using multiple window sizes.
    """
    tA, cA = get_raw_buffers(za, target)
    tB, cB = get_raw_buffers(zb, target)
    if tA is None or len(tA) < 5000: return 0, None, 0
    
    limit = min(len(tA), len(tB), 50000) # Deep scan
    tA, cA, tB, cB = tA[:limit], cA[:limit], tB[:limit], cB[:limit]
    
    # --- WIDE SYNC SEARCH ---
    diffs = (tB[:10000] * 1e9)[:,None] - (tA[:10000] * 1e9)[None,:]
    
    # 20k bins = 10ns resolution
    ...
    if peak_height < 10: return 0, None, peak_height
    ...
    fine_shift = (edges_fine[np.argmax(hist_fine)] + edges_fine[np.argmax(hist_fine)+1])/2.0 * 1e-9

    local_best_S = 0.0
    local_best_cfg = None
    
    # Pre-calc shifted matrix for speed
    ...
    for s_bit in [0, 1, 2]:
        for d_bit in [0, 1, 2]:
            ...
            # WINDOW SCAN: Try different tightness
            for win_ns in [2.5, 6.0, 12.0]:
                pairs = np.argwhere(np.abs(dt_matrix) < win_ns)
                ...
                if abs(S) > local_best_S:
                    ...
                    local_best_cfg = (s_bit, d_bit, flip, win_ns) # Save best window too
                
    return local_best_S, local_best_cfg, peak_height

def calibrate_bitmask(za, zb):
    print(f"[CALIBRATION] Priority Scan with Window Optimization...")
    ...
    candidates = (priority + [c for c in common if "sine" in c])[:30]
    
    global_best_S = 0.0
    # Store window in config: (s, d, f, window)
    global_best_cfg = (0, 1, 1, 2.5) 
    
    for target in candidates:
        S, cfg, height = check_file_for_S(za, zb, target)
        
        status = f"S={S:.4f} (Win={cfg[3]}ns)" if S > 0 else "No S"
        if height > 50:
...
def find_shift_and_sync(tA, tB):
    ...
    peak_val = hist[np.argmax(hist)]
    if peak_val < np.mean(hist) * 3: return None, 0 
    shift = (edges[np.argmax(hist)] + edges[np.argmax(hist)+1])/2.0 * 1e-9
    return shift, peak_val

def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Universal Miner v09...")
    ...
    with zipfile.ZipFile(alice_zip, "r") as za, zipfile.ZipFile(bob_zip, "r") as zb:
        s_bit, d_bit, flip, win_ns = calibrate_bitmask(za, zb)
        print(f"  Locked Window Size: {win_ns} ns")
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v09")
    print("==========================================")
