...
"""
weihs_topology_lab_v5.py

Fixes:
  - 'reverse-bits' flag now toggles correctly (default=False).
  - Auto-resolves stem names.
  - Robust Zip handling.

USAGE:
  1. CALIBRATE (Find the signal):
     python .\weihs_topology_lab_v5.py --alice-zip "..\Alice.zip" --bob-zip "..\Bob.zip" --folder longdist calibration --stem longdist20 --cal-min -5 --cal-max 5

  2. ANALYZE (Run the topological test):
       python .\weihs_topology_lab_v5.py --alice-zip "..\Bell_data\7185335\Alice.zip" --bob-zip "..\Bell_data\7185335\Bob.zip" --folder longdist --window-ns 1.0 --reverse-bits calibration --stem longdist20 --cal-min -2 --cal-max 2


python .\weihs_topology_lab_v5.py   --alice-zip "..\Bell_data\7185335\Alice.zip"   --bob-zip   "..\Bell_data\7185335\Bob.zip"   --folder longdist0   --window-ns 1.0   --reverse-bits   analysis   --delta-ns 4.5        
"""
...
def resolve_stem(stems: List[str], user_input: str) -> str:
    """Auto-finds full path for stem."""
    if user_input in stems:
        return user_input
    matches = [s for s in stems if s.endswith(user_input) or s.endswith("/" + user_input)]
    if not matches:
        raise FileNotFoundError(f"Could not find stem '{user_input}'. Available sample: {stems[:3]}")
    return matches[0]
...
def load_events(z: zipfile.ZipFile, stem: str, reverse_bits: bool) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not reverse_bits:
        # Standard: bit 0 = detector (outcome), bit 1 = switch (setting)
        det = (c & 1).astype(np.uint8, copy=False)
        s   = ((c >> 1) & 1).astype(np.uint8, copy=False)
    else:
        # Reversed: bit 1 = detector, bit 0 = switch
...
def run_calibration(args, stems, za, zb):
    ...
    if args.stem:
        ...
        target_stem = stems[0]
        print(f"Using stem: '{target_stem}'")
        
    print(f"Config: reverse_bits={args.reverse_bits}")
...
def main():
    ...
    # FIXED: Default is False, so presence of flag turns it ON.
    ...
    with zipfile.ZipFile(args.alice_zip, "r") as za, zipfile.ZipFile(args.bob_zip, "r") as zb:
        ...
        if not stems:
            print(f"No stems found matching '{args.folder}'.")
            return
