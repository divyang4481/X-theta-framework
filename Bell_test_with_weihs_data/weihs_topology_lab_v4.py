...
Updates in v4:
  - Auto-resolves stem names (e.g. 'longdist20' -> 'Timetags/longdist/longdist20')
  - Robust Zip path handling

USAGE:
  python src/weihs_topology_lab_v4.py --alice-zip "..\Alice.zip" --bob-zip "..\Bob.zip" --folder longdist calibration --stem longdist20
  python src/weihs_topology_lab_v4.py --alice-zip "..\Alice.zip" --bob-zip "..\Bob.zip" --folder longdist analysis --delta-ns 1.5
"""
...
def _norm_member(name: str) -> str:
    """Normalize zip member names to internal standard (forward slashes, no leading roots)."""
    n = name.replace("\\", "/").lstrip("/")
    parts = n.split("/")
    # If the zip has a top-level 'Alice' or 'Bob' folder, strip it
    if parts and parts[0].lower() in ("alice", "bob"):
        n = "/".join(parts[1:])
    return n

def _zip_name_map(z: zipfile.ZipFile) -> Dict[str, str]:
    """Map normalized lowercase path -> original member name in zip."""
    m: Dict[str, str] = {}
...
def list_stems(z: zipfile.ZipFile, folder_filter: Optional[str]) -> List[str]:
    """Return unique stems (paths without _V.dat/_C.dat suffixes)."""
    ...
    for n in names:
        ...
        if nl.endswith("_v.dat"):
            have_v.add(n[:-6]) # Remove suffix
        elif nl.endswith("_c.dat"):
            have_c.add(n[:-6])

    return sorted(have_v.intersection(have_c))

def resolve_stem(stems: List[str], user_input: str) -> str:
    """Finds the full internal path for a short user input."""
    # 1. Exact match
    if user_input in stems:
        return user_input
    
    # 2. Suffix match (e.g. 'longdist20' matching 'Timetags/longdist/longdist20')
    matches = [s for s in stems if s.endswith(user_input) or s.endswith("/" + user_input)]
    
    if not matches:
        raise FileNotFoundError(f"Could not find stem '{user_input}' in the dataset.\nAvailable stems (first 5): {stems[:5]}")
...
def load_events(z: zipfile.ZipFile, stem: str, reverse_bits: bool) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # Debug print if missing
    if v_key not in nm or c_key not in nm:
        print(f"DEBUG: Looking for keys: {v_key}, {c_key}")
        # print(f"DEBUG: Available keys sample: {list(nm.keys())[:5]}")
...
def run_calibration(args, stems, za, zb):
    ...
    # Auto-resolve the short name "longdist20" to "Timetags/longdist/longdist20"
    if args.stem:
        target_stem = resolve_stem(stems, args.stem)
        print(f"Resolved stem: '{args.stem}' -> '{target_stem}'")
    else:
        target_stem = stems[0]
        print(f"No stem specified, using first available: '{target_stem}'")
    ...
    # Using smaller steps if range is small, or user arg
...
def main():
    ...
    with zipfile.ZipFile(args.alice_zip, "r") as za, zipfile.ZipFile(args.bob_zip, "r") as zb:
        ...
        if not stems:
            print(f"No stems found matching folder '{args.folder}' in both zips.")
            return
