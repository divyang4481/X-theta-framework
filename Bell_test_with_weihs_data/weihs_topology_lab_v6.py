...
"""
weihs_topology_lab_v6.py

Updates in v6:
  - Added '--stem' support to ANALYSIS mode (critical for targeting single files).
  - Fixed 'folder' filtering logic to be robust.

USAGE:
  1. Real Physics Run:
     python .\weihs_topology_lab_v6.py     --alice-zip "..\Bell_data\7185335\Alice.zip"     --bob-zip   "..\Bell_data\7185335\Bob.zip"     --folder longdist     --window-ns 1.0     --reverse-bits     analysis     --delta-ns 4.5     --stem longdist0

  2. ANALYZE (Targeting specific file):
     python .\weihs_topology_lab_v6.py   --alice-zip "..\Bell_data\7185335\Alice.zip"   --bob-zip   "..\Bell_data\7185335\Bob.zip"   --folder longdist   --window-ns 1.0   --reverse-bits   analysis   --delta-ns 4.5   --stem longdist0   --null-shuffle
"""
...
def run_analysis(args, stems, za, zb):
    ...
    # Filter stems if requested (NEW IN V6)
    if args.stem:
        try:
            target = resolve_stem(stems, args.stem)
            stems = [target]
            print(f"Targeting single stem: {target}")
...
def main():
    ...
    p_ana.add_argument("--stem", help="Specific stem (optional)")
