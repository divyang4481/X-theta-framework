#!/usr/bin/env python3
"""
weihs_universal_pipeline_v05.py

THE UNIFIED DISCOVERY PIPELINE (Entropy-Aware Calibration)
================================================================================
CHANGES FROM v03:
  1. ENTROPY CHECK: During calibration, we explicitly reject bitmasks where 
     the outcomes are constant (stuck bits). This prevents the "S=2.0000" 
     artifact caused by calculating correlations on unused/zeroed bits.
  2. MULTI-FILE SCAN: Iterates through files until it finds a valid, 
     high-entropy signal with S > 1.8.

USAGE:
   python weihs_universal_pipeline_v05.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
CACHE_FILE = "weihs_mined_data_v5.npz"
...
def check_file_for_S(za, zb, target):
    ...
    REJECTS configurations with low entropy (stuck bits).
    """
    ...
    for s_bit in [0, 1, 2]:
        for d_bit in [0, 1, 2]:
            ...
            # --- ENTROPY CHECK ---
            # If outcomes never change, this is a stuck bit.
            # Variance should be > 0.
            if np.std(oA) < 0.1 or np.std(oB) < 0.1:
                continue 
...
def calibrate_bitmask(za, zb):
    print("[CALIBRATION] Scanning files for Bell Violation (High Entropy)...")
...
def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Universal Miner v05...")
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v05 (Entropy Fix)")
    print("==========================================")
