#!/usr/bin/env python3
"""
weihs_universal_pipeline_v08.py

THE UNIFIED DISCOVERY PIPELINE (The "Longdist" Priority Fix)
================================================================================
PHYSICS VERIFICATION:
  - Theta (Geometric Phase) is calculated purely from the topology of setting 
    choices (Sequence of A/B). It is strictly TIME-INDEPENDENT.
  - Timestamps are used ONLY for coincidence synchronization (pairing photons).

CHANGES FROM v07:
  1. PRIORITY TARGETING: Forces the calibration tournament to check 'longdist' 
     files FIRST. (Previous versions failed because they got stuck checking 
     'sine' alignment files which have S=0).
  2. HIGH-RES SYNC: Uses 20,000 bins for the coarse sync search (10ns width) 
     to ensure the correlation peak is found even in a massive +/- 100us window.
  3. INTELLIGENT FAILSAFE: Defaults to the known physical config (Set=0, Det=1) 
     if calibration yields no results, rather than a random guess.

USAGE:
   python weihs_universal_pipeline_v08.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
BIN_COUNT = 6               
CALIB_WINDOW_NS = 100000.0  # +/- 100us Wide Net
CACHE_FILE = "weihs_mined_data_v8.npz"

# ==========================================
# PART 1: TARGETED CALIBRATION
# ==========================================
...
def check_file_for_S(za, zb, target):
    """
    Helper: Tries to calculate max S using WIDE search and High Res.
    """
    tA, cA = get_raw_buffers(za, target)
    tB, cB = get_raw_buffers(zb, target)
    if tA is None or len(tA) < 2000: return 0, None, 0
    
    limit = min(len(tA), len(tB), 25000)
    tA, cA, tB, cB = tA[:limit], cA[:limit], tB[:limit], cB[:limit]
    
    # --- WIDE SYNC SEARCH (High Res) ---
    diffs = (tB[:5000] * 1e9)[:,None] - (tA[:5000] * 1e9)[None,:]
    
    # 20k bins = 10ns resolution over 200us range
    hist, edges = np.histogram(diffs, bins=20000, range=(-CALIB_WINDOW_NS, CALIB_WINDOW_NS))
    peak_bin = np.argmax(hist)
    peak_val_ns = (edges[peak_bin] + edges[peak_bin+1])/2.0
    peak_height = hist[peak_bin]
    
    # SNR Check (Local background)
    bg_start = max(0, peak_bin - 100)
    bg_end = min(len(hist), peak_bin + 100)
    bg_level = np.mean(hist[bg_start:bg_end])
    if peak_height < max(5, bg_level * 2): 
        return 0, None, peak_height # Weak signal
    ...
    peak_fine_idx = np.argmax(hist_fine)
    fine_shift = (edges_fine[peak_fine_idx] + edges_fine[peak_fine_idx+1])/2.0 * 1e-9
...
def calibrate_bitmask(za, zb):
    print(f"[CALIBRATION] Priority Scan (Targeting 'longdist')...")
    ...
    # STRICT PRIORITY: Check longdist FIRST. Ignore sine unless desperate.
    priority = [c for c in common if "longdist" in c]
    candidates = (priority + [c for c in common if "sine" in c])[:50]
    
    global_best_S = 0.0
    global_best_cfg = (0, 1, 1) # Fallback to v19 config (Set=0, Det=1)
...
def find_shift_and_sync(tA, tB):
    ...
    # Mining uses 20us window
    valid = diffs[np.abs(diffs) < 20000.0] 
...
def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Universal Miner v08...")
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v08")
    print("==========================================")
