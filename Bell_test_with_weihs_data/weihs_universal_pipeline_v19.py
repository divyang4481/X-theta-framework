#!/usr/bin/env python3
"""
weihs_universal_pipeline_v19.py

THE UNIFIED DISCOVERY PIPELINE (Deep Calibration & Expanded Bit Scan)
================================================================================
CRITICAL UPGRADE:
  - v18 failed (S=0.7) because it restricted the search to bits 0 and 1.
  - v19 expands the calibration scan to Bits 0, 1, 2, 3.
  - It also prints detailed SNR (Signal-to-Noise) stats for the time sync 
    to prove we are locking onto photons, not random noise.

USAGE:
   python weihs_universal_pipeline_v19.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
CACHE_FILE = "weihs_mined_data_v19.npz"
...
def find_shift_and_sync(tA, tB):
    ...
    # SNR Check
    bg_mask = np.ones(len(hist), dtype=bool)
    bg_mask[max(0, peak_bin-5):min(len(hist), peak_bin+6)] = False
    bg_mean = np.mean(hist[bg_mask]) if np.any(bg_mask) else 1.0
    snr = peak_val / (bg_mean + 0.1)
    
    if snr < 3.0: return None # Strict Sync requirement
    ...
    return shift, snr, peak_val, bg_mean
...
def calibrate_best_file(za, zb, stems):
    print(f"  [CALIBRATION] Deep Scan of {len(stems)} files (Bits 0-3)...")
    
    global_max_S = 0.0
    global_best_cfg = (0, 1, 1) # Default
    
    # EXPANDED BIT SCAN (0, 1, 2, 3)
    bits_to_scan = [0, 1, 2, 3]
    ...
    for stem in stems:
        ...
        shift, snr, peak, bg = res
        
        # Only check files with clear optical sync
        if snr < 5.0: 
            # print(f"    Skip {stem}: Weak Sync (SNR={snr:.1f})")
            continue
        ...
        # Brute Force Bits
        for s in bits_to_scan:
            for d in bits_to_scan:
                if s == d: continue
                ...
                if np.std(oA) < 0.1 or np.std(oB) < 0.1: continue
                ...
                if abs(S) > global_max_S:
                    ...
                    print(f"    New Leader: {stem} -> S={global_max_S:.4f} (Set={s}, Det={d}, SNR={snr:.1f})")
...
def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Universal Miner v19...")
    ...
    with zipfile.ZipFile(alice_zip, "r") as za, zipfile.ZipFile(bob_zip, "r") as zb:
        ...
        for i, stem in enumerate(stems):
            ...
            if kept_files % 5 == 0:
                print(f"  Accepted {kept_files} files (Last S={file_S:.2f}). Total: {total_events}")
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v19")
    print("==========================================")
    ...
    if g_S < 1.5:
        print("\nWARNING: Global S is below 2.0. This may indicate poor quality data.")
