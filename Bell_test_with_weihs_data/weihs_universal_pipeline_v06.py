#!/usr/bin/env python3
"""
weihs_universal_pipeline_v06.py

THE UNIFIED DISCOVERY PIPELINE (Tournament Calibration)
================================================================================
CHANGES FROM v05:
  1. TOURNAMENT LOGIC: Instead of aborting if no file hits S > 1.8, the script 
     now tracks the 'Best Candidate' across all scanned files and uses the 
     winner. This handles noisy files where local S might dip to ~1.5.
  2. INCREASED SCAN DEPTH: Checks 50 files instead of 20 to ensure a good 
     calibration target is found.
  3. DATA SAFETY: Ignores files with < 1000 events during calibration to 
     prevent statistical flukes.

USAGE:
   python weihs_universal_pipeline_v06.py --alice-zip "A.zip" --bob-zip "B.zip"
================================================================================
"""
...
BIN_COUNT = 6               
SYNC_WINDOW_NS = 2000.0     
CACHE_FILE = "weihs_mined_data_v6.npz"

# ==========================================
# PART 1: TOURNAMENT CALIBRATION
# ==========================================
...
def check_file_for_S(za, zb, target):
    ...
    if tA is None or len(tA) < 2000: return 0, None
    
    limit = min(len(tA), len(tB), 15000) # Deeper look
...
def calibrate_bitmask(za, zb):
    print("[CALIBRATION] Tournament Mode: Searching 50 files for best signal...")
    ...
    global_best_S = 0.0
    global_best_cfg = (1, 0, 1) # Fallback
    ...
    for target in candidates:
        ...
        if S > global_best_S:
            global_best_S = S
            global_best_cfg = cfg
            print(f"  New Leader: {target} -> S={S:.4f} (Set={cfg[0]}, Det={cfg[1]})")
            
            if S > 2.0: # Early exit if we find a violation
                print(f"  VIOLATION FOUND! Locking configuration.")
                return cfg
            
    print(f"  Tournament Finished. Winner S={global_best_S:.4f}. Using Config: {global_best_cfg}")
...
def mine_data(alice_zip, bob_zip):
    print("\n[MINING] Starting Universal Miner v06...")
...
def main():
    ...
    print("==========================================")
    print("   WEIHS UNIVERSAL PIPELINE v06")
    print("==========================================")
